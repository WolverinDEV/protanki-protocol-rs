...
use tank_bot_rs::{TanksClient, packet_handler::{self, PacketHandler}, Task, packets::{Packet, PacketDowncast, self}, codec::{BattleTeam, LayoutState, MoveCommand, RotateTurretCommand}, PacketDebugFilter, SimplePacketDebugFilter};
...
struct TaskSimpleAction<
    E: FnOnce(&mut TanksClient) -> anyhow::Result<()>,
    M: Fn(&mut TanksClient, &dyn Packet) -> anyhow::Result<Option<R>>,
    R: Send
> {
    executor: Option<E>,
    matcher: M,
...
impl<
    E: (FnOnce(&mut TanksClient) -> anyhow::Result<()>) + Send,
    M: (Fn(&mut TanksClient, &dyn Packet) -> anyhow::Result<Option<R>>) + Send,
    R: Send
...
> Task for TaskSimpleAction<E, M, R> {
    ...
    fn poll(&mut self, client: &mut TanksClient, cx: &mut std::task::Context) -> std::task::Poll<anyhow::Result<Self::Result>> {
...
impl PacketHandler for PacketHandlerRandomMoveControlFlags {
    fn poll(&mut self, client: &mut TanksClient, cx: &mut std::task::Context) -> Poll<anyhow::Result<()>> {
...
impl PacketHandlerTankSpawner {
    ...
    fn update(&mut self, state: &mut LocalTankState, client: &mut TanksClient, cx: &mut std::task::Context) -> anyhow::Result<Option<LocalTankState>> {
...
impl PacketHandler for PacketHandlerTankSpawner {
    fn handle_packet(&mut self, _client: &mut TanksClient, packet: &dyn Packet) -> anyhow::Result<()> {
...
impl PacketHandler for BattleTanksPacketHandler {
    ...
    fn poll(&mut self, _client: &mut TanksClient, _cx: &mut std::task::Context) -> Poll<anyhow::Result<()>> {
        Poll::Pending
    }
}
